...
namespace SR_Editor.Extension
{
    ...
    public class CustomSyntaxHighlightService : ISyntaxHighlightService
    {
        ...
        Regex _paramString = new Regex(@"(?i)(@[a-zA-Z]+)");
        Regex _commentedString = new Regex(@"(?i)(/\*([^*]|[\r\n]|(\*+([^*/]|[\r\n])))*\*+/)");

        public CustomSyntaxHighlightService(Document document)
        {
            this.document = document;
            string[] keywords = { "INSERT","COUNT","SUM","HAVING","GROUP","BY", "NOLOCK", "INNER","CASE","IN", "JOIN","AS","THEN","WHEN","END", "UPDATE", "SELECT", "CREATE", "TABLE", "USE", "IDENTITY", "ON", "OFF", "NOT", "NULL", "WITH", "SET", "GO", "DECLARE", "EXECUTE", "NVARCHAR", "FROM", "INTO", "VALUES", "WHERE", "AND" };
            this._keywords = new Regex(@"(?i)\b(" + string.Join("|", keywords.Select(w => Regex.Escape(w))) + @")\b");
            
            string[] database = { "SR_SHARD","SR_R_SHARD","SR_R_SHARDLOG","SR_SHARDLOG","SR_ACCOUNT","PANEL","F8FILTER","REMOLOGGER" };
            this._database = new Regex(@"(?i)\b(" + string.Join("|", database.Select(w => Regex.Escape(w))) + @")\b");
        }
        ...
        private List<SyntaxHighlightToken> ParseTokens()
        {
            ...
            ranges = document.FindAll(_database).GetAsFrozen() as DocumentRange[];
            for (int j = 0; j < ranges.Length; j++)
            {
                if (!IsRangeInTokens(ranges[j], tokens))
                    tokens.Add(CreateToken(ranges[j].Start.ToInt(), ranges[j].End.ToInt(), Color.DarkBlue));
            }
            ...
            //Find all params
            ranges = document.FindAll(_paramString).GetAsFrozen() as DocumentRange[];
            for (int j = 0; j < ranges.Length; j++)
            {
                if (!IsRangeInTokens(ranges[j], tokens))
                    tokens.Add(CreateToken(ranges[j].Start.ToInt(), ranges[j].End.ToInt(), Color.Purple));
            }
